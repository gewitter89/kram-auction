...
export default function AdminPage() {
  ...
  const [logs, setLogs] = useState<string[]>([
    ...
    "[12:43:00] SYSTEM: Перевірка журналу подій завершена. Розбіжностей не виявлено."
  ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const warningLogs = [
        ...
        "SYSTEM: Завершено перевірку контрольної суми журналу подій."
      ];
      const ddosLogs = [
  ...
  const handleClearAllData = async () => {
    soundService.playClick();
    if (confirm("Ви впевнені, що хочете скинути всі дані платформи KRAM.UA? Це очистить історію ставок і створені лоти.")) {
      localStorage.clear();
